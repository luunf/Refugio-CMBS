...
interface Props {
  ...
  onClose: () => void;
  onEditado: () => void;
  registro: RegistroHistorial | null;
}
...
export default function ModalEditarHistorial({ visible, onClose, onEditado, registro }: Props) {
  ...
  // Precarga los valores del registro cuando el modal se abre (o cambia el registro)
  useEffect(() => {
    if (visible && registro) {
      setEstadoId(registro.estado_id);
      setPersonaId(registro.persona?.id_persona ?? null);
      setFechaDesde(registro.fecha_desde);
      setFechaHasta(registro.fecha_hasta ?? "");
      setEsActual(!registro.fecha_hasta);
    }
  }, [visible, registro]);
  ...
  const handleGuardar = async () => {
    ...
    setLoading(true);
    try {
      await api.updateHistorial(registro.id, {
        estado_id: estadoId,
      ...
      onEditado();
      handleClose();
      Alert.alert(t("success"), t("successEditarHistorial"));
    } catch (e: any) {
      Alert.alert(t("error"), e?.response?.data?.error ?? t("errorEditarHistorial"));
    } finally {
      setLoading(false);
    }
  };
